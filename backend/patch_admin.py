...
new_count_span = """<span className={`rounded-full px-1.5 py-0.5 text-[10px] ${active ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'}`}>{count}</span>"""

content = content.replace(old_count_span, new_count_span)

# Fix 2: Add check for privacy to "设为精选" button:
