...
start_str_1 = """            <div className="p-6 space-y-6">
              <div>
...
new_str_1 = """            <div className="p-6 space-y-6 overflow-hidden">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <div className="flex items-center max-w-full">
                  <div className="bg-gray-50 flex-1 p-3 rounded-l-lg text-sm text-gray-600 overflow-x-auto scrollbar-hide border border-gray-200 border-r-0 select-all font-mono whitespace-nowrap min-w-0">
                    {typeof window !== 'undefined' ? window.location.origin : ''}/datasets/{encodeURIComponent(dataset.owner?.username || '')}/{dataset.slug}{(dataset.is_password_protected && (dataset.access_password || sharePassword.trim())) ? `?share_token=${sharePassword.trim() || dataset.access_password}` : ''}
                  </div>
                  <button
...
content = replace_between(start_str_1, end_str_1, new_str_1, content)


start_str_2 = """              {dataset.is_password_protected && (
...
new_str_2 = """              {dataset.is_password_protected && (
                <div className="relative overflow-hidden rounded-xl border border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50/30">
                  <div className="p-5">
...
content = replace_between(start_str_2, end_str_2, new_str_2, content)
