...
old_block = """      {/* Tabs + create button */}
      <div className="flex justify-between items-center mb-4">
        ...
        {user.role === 'admin' ? (
          <Link href="/admin" className="bg-primary text-primary-foreground shadow hover:bg-primary/90 px-4 py-2 rounded-md text-sm font-medium transition-colors">
            进入管理后台
          </Link>
        ) : (
          <Link href="/upload" className="bg-primary text-primary-foreground shadow hover:bg-primary/90 px-4 py-2 rounded-md text-sm font-medium transition-colors">
            创建新数据集
          </Link>
        )}
      </div>"""

new_block = """      {/* Tabs + create button */}
...
if old_block in content:
    ...
    print("WARNING: TABS BLOCK NOT FOUND. TRYING REGEX.")
    # attempt regex
    regex_res = re.sub(r'      \{\/\* Tabs \+ create button \*\/\}.*?      <\/div>', new_block, content, flags=re.DOTALL)
    if regex_res != content:
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(regex_res)
        print("TABS REPLACED VIA REGEX!")
    else:
        print("TOTAL FAIL")
